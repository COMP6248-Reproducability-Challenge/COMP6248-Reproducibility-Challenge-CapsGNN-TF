...
def settings():
    ...
    parser.add_argument("--dataset_input_dir", type=str, default="../graph_gexf/IMDB-BINARY",help="Where gexf dataset is stored.")
...
class GraphDataset():
    def __init__(self,input_dir,extn,class_label_fname,dataset_output_dir,attri_dict):
        ...
        # graph to label is a key-value pair eg.{0.gexf: 6}
    ...
    def scan(self,graphs_dataset):
        ...
        print('Dataset Scan:-----------------------')
        attri_to_id_maps = []
        label_to_id_map = {} # 6 5 1 3 2 4 just label types
        id_to_label_map = []
        id_to_attri_maps = []
        degree_max = 0
        # add label info to label_to_id_map for constructing label id mapping correlation
        # after this for loop: label_to_id_map = {} 6 5 1 3 2 4 just label types
        ...
        num_edge = []
        num_node = []
        # generating graph id and file from datasets
        for graph_idx, graph_file in enumerate(graphs_dataset):
            # get gexf data from folder and the output type is a graph structure.
            graph = nx.read_gexf(graph_file)
            # add graph info number of edge and number of node into their related list.
            num_edge.append(graph.number_of_edges())
            num_node.append(len(graph))
            # begin to add attribution value to related var
            attri_idx = 0; attri_idx_all = 0
            # for loop to find attri_dict: label, degree, constant
            for attri_name in self.attri_dict:
                # label's channel is 1.
                num_atti_channel = self.attri_dict[attri_name]['channel']
                # continue loop without adding degree into atti_channel
                if attri_name == 'degree':
                    continue
                # for loop for traversing all
                for attri_idx_sub in range(num_atti_channel):
                    print(attri_name)
                    attri_idx_all = attri_idx_sub+attri_idx
                    # extract data value under graph structure and save it into nodes
                    nodes = graph.nodes(data=attri_name)
                    # check node information to see if any illegal value
        ...
        # calculate and check basic values in the data-set.
        ...
        # last time we did;t deal with the degree one
        for attri_name in self.attri_dict:
            if attri_name == 'degree':
    ...
    def data_gen(self,pickle_v,save):
        ...
        :return:
        """
        # the dict contains 3 members label degree and constant
        # Here we ignore the label with 'degree' so we have 2 instead of 3.
        ...
        for graph_idx, graph_file in enumerate(self.graphs_dataset):
            ...
            node_attri_recon = []
            node_attri_input = []
            # read data from gexf and put it into a graph structure
            graph = nx.read_gexf(graph_file)
            """
            Transfer the graph structure into a node list.
            this list is the value node label or id in .gexf files. [27, 36, ...]
            <node id="27" label="27">      <node id="36" label="36"> ...
            """
            nodelist = list(graph)
            # processing the node value stored in the nodelist var.
            ...
            for node in nodelist:
                ...
                for attri_name in self.attri_dict:
                    num_atti_channel = self.attri_dict[attri_name]['channel']
                    # still deal with data without label cato
                    if attri_name == 'degree':
                        continue
                    try:
                        # build graph node
    ...
    def buil_input_index(self):
        input_index = []
        attri_idx = 0
        # obtain attri idx without degree label data
        for attri_name in self.attri_dict:
            num_atti_channel = self.attri_dict[attri_name]['channel']
            ...
            attri_idx = attri_idx_all + 1
        # obtain arrti idx with degree label
        for attri_name in self.attri_dict:
            if attri_name == 'degree':
                num_channel = self.attri_dict[attri_name]['channel']
                if_input = self.attri_dict[attri_name]['if_input']
                for channel_idx in range(num_channel):
                    ...
                    if if_input:
                        input_index.append(attri_idx_all)
        # [label node data, constant node data, degree label data]
        return input_index
