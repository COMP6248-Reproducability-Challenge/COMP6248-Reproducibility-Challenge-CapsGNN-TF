    ...
    for train_test_split_groups_idx,groups_dict in enumerate(train_test_split_groups):
        ...
        with tf.Session(graph=GraphNet.graph,config=tf_config) as sess:
            ...
            for i in range(FLAGS.epochs):
                loss_error_record = []
                loss = 0;error = 0;loss_margin = 0;loss_regular = 0;train_acc = 0
                ...
                while one_epoch == 0:
                    ...
                    _, loss_val, loss_regular_val, loss_margin_val, error_val, lr, not_equal, train_acc_val, graph_capsule_out = sess.run(
                        [GraphNet.optimizer,
                         GraphNet.loss,
                         ...
                         GraphNet.not_equal,
                         GraphNet.acc,
                         GraphNet.graph_capsules,
                         GraphNet.classes_capsules,
                        ],
                        feed_dict=feed_dict)
                    # print(not_equal)
                    print('gloabal_step:{}'.format(lr))
                    print('***************************')
                    print(graph_capsule_out[0].shape)
                    print(graph_capsule_out[0])
                    # 32 class
                    # print(caps_class)
                    #
                    loss += loss_val; loss_margin+= loss_margin_val;loss_regular+=loss_regular_val;train_acc+= train_acc_val
                    error += error_val
                    step += 1
                    ...
                    if step % 1 == 0:
                        if step > 0:
                            ...
                            average_accuracy = train_acc / step
                            tf.logging.info('Epoch : %d : Batch size: %d Average loss for step: %d : %f Average accuracy for step: %d: %f' % (i, len(label), step, average_loss, step, average_accuracy))
                            tf.logging.info('  Margin loss : %f Regular loss: %f' % (average_loss_margin,average_loss_regular))

                epoch_time = time() - t0
                tf.logging.info('######################### TRAIN\tEpoch: %d : loss : %f, error : %f, Acc: %f, time : %.2f sec.  #####################' % (i, loss_margin / step, error/(1.0*processed_graph_num), train_acc/step, epoch_time))
                loss_error_record.extend([' train : ', str(loss_margin / step), str(error / (1.0 * processed_graph_num)), '\n'])
                # For test set
                if i % 1 == 0:
                    test_step = train_step
                    loss = 0; error = 0;test_acc=0
                    one_epoch = 0;step = 0
                    ...
                    while one_epoch == 0:
                        ...
                        loss_val,error_val, acc_val = sess.run(
                            [GraphNet.loss_margin,
                             GraphNet.error,
                             GraphNet.acc],
                            feed_dict=feed_dict)

                        loss += loss_val; error += error_val; step += 1;test_acc += acc_val; test_step += 1;

                    test_time = time()-t0
                    tf.logging.info('#########################  TEST\tEpoch: %d : loss : %f, error : %f, Acc : %f, time : %.2f sec.  #####################' % (i, loss / step ,error/(1.0*len(gd.graphs_dataset_test)), test_acc / step,test_time))
